...
if ($transport->xpdo) {
    ...
    switch ($options[xPDOTransport::PACKAGE_ACTION]) {
        ...
        case xPDOTransport::ACTION_UPGRADE:
            $modelPath = $modx->getOption('tevent_core_path', null,
                    $modx->getOption('core_path') . 'components/tevent/') . 'model/';
            $modx->addPackage('tevent', $modelPath);

            $manager = $modx->getManager();
            $objects = array();
            $schemaFile = MODX_CORE_PATH . 'components/tevent/model/schema/tevent.mysql.schema.xml';
            ...
            foreach ($objects as $tmp) {
                ...
                if ($newTable) {
                    ...
                    /* Чтоб не удалять кастомные поля
